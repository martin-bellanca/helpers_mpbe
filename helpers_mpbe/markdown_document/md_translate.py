...
# Kivy imports --------------------------------------------------------------
# kivy_dkw ------------------------------------------------------------------
from helpers_mpbe.markdown_document.markdown_extension import TaskListMDExtension, WhiteLineMDExtension
...
def title_font_size(level: int) -> int:
    """
    Tamaño de fuente con que se renderiza un título de nivel `level` (1-6).
    Disminuye el tamaño según el nivel. Única fuente de verdad: la usan
    _ExtTitle (render) y los editores de línea (wg_markdown2) para matchear
    la fuente del input con la del render.
    """
    return 40 - (level - 1) * 5
