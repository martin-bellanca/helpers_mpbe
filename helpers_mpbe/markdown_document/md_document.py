...
from helpers_mpbe.markdown_document import TYPE_PATTERNS
from helpers_mpbe.markdown_document.md_translate import TranslateMarkdownToKVMarkup, ExtKVTable
from helpers_mpbe.markdown_document.md_labels import MDTextLabel, MDSeparatorLabel, MDHeadLabel, MDTaskLabel, MDToDoLabel
from helpers_mpbe.markdown_document.md_labels import MDTableLabel, MDImageLabel, MDCodeLabel
...
class MDLine(EventDispatcher):
    ...
    **Events:**
    ...
    def __init__(self, md_text, prev_line, next_line, type=MD_LINE_TYPE.TEXT, num_line=-1):
        ...
        """
        self.md_text = md_text  # compose(md_text, str, None)
        self.type = type  # compose(type, MD_LINE_TYPE, None)
    ...
    def is_title(self) -> bool:
        '''Devuelve True si la linea es un titulo'''

        '''Devuelve el nivel de título'''
        if self.type in [MD_LINE_TYPE.TITLE, MD_LINE_TYPE.HEAD_TITLE, MD_LINE_TYPE.UNDERLINE_TITLE] or self.md_text.startswith('#'):
            return True
        else:
            return False
    ...
    def get_title_first_child(self) -> MDLine|None:
    ...
    def get_title_parent(self) -> MDLine|None:
    ...
    def get_title_prev_same_level(self) -> MDLine|None:
    ...
    def get_title_next_same_level(self) -> MDLine|None:
    ...
    def get_title_prev(self) -> MDLine|None:
    ...
    def get_title_next(self) -> MDLine|None:
    ...
    @num_line.setter
    def num_line(self, new_num_line):
        self._num_line = new_num_line
        '''Actualizacion automatica de los numeros de linea'''
        if self.next_line:
            # print(f"  value= {new_num_line}-> {self.md_text}")
            self.next_line.num_line = new_num_line + 1
    
    def set_num_line(self, new_num_line):
        '''Asigna el numero de linea SIN actualizar el numero de linea de las siguientes lineas'''
        self._num_line = new_num_line
